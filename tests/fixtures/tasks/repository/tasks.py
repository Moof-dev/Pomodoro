...
class FakeTaskRepository:
    ...
    async def get_tasks(self):
        return [self.task_models_factory.build() for _ in range(5)]

    async def get_task(self, task_id: int):
        return self.task_models_factory.build(id=task_id)
    ...
    async def update_task_name(self, task_id: int, name: str):
        return self.task_models_factory.build(id=task_id, user_id=1, name=name)
