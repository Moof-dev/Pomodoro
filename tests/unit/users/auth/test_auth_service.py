...
async def test_login__success(mock_auth_service, mock_user_repository):
    username = "true_username"
    password = "true_password"
    user = await mock_user_repository.get_user_by_username(username)
    user.password = password
    mock_auth_service._validate_auth_user(user=user, password=password)
    access_token = mock_auth_service.generate_access_token(user_id=user.id)

    assert mock_auth_service.get_user_id_from_access_token(access_token) == user.id

async def test_get_google_redirect_url__success(mock_auth_service, settings):
...
async def test_get_google_redirect_url__fail(mock_auth_service, settings):
...
async def test_google_auth__success(mock_auth_service):
    code="fake_code"
...
async def test_generate_access_token__success(mock_auth_service, settings):
    user_id = 1
...
async def test_get_user_id_from_access_token__success(mock_auth_service):
    user_id = 1

    access_token = mock_auth_service.generate_access_token(user_id=user_id)

    assert mock_auth_service.get_user_id_from_access_token(access_token) == user_id
